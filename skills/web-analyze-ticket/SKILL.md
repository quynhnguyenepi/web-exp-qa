...
1. **Pre-flight:** Validate ticket ID, check MCP connectivity (use ToolSearch to load MCP tools)
2. **Fetch JIRA context DIRECTLY (sequential, required first):**
   - Call `mcp__atlassian__jira_get_issue` with `fields: "*all"`, `expand: "renderedFields"`, `comment_limit: 20`
   - Call `mcp__atlassian__jira_get_issue` again with `fields: "issuelinks,parent,customfield_10014"` to get links + epic
   - If parent/epic exists: call `mcp__atlassian__jira_search` with `jql: "parent = {EPIC_KEY}"` to get sibling tickets
   - Extract URLs (Confluence, Figma) from description + comments
   - **Do NOT use Agent tool for this step** -- direct MCP calls are faster and more reliable
3. **Download and read attachments DIRECTLY (required if attachments exist):**
   - Call `mcp__atlassian__jira_get_issue_images` with the ticket key to download all image attachments
   - This returns inline images that can be analyzed visually (screenshots of UI, event payloads, etc.)
   - **ALWAYS do this** -- attachments often contain critical context (screenshots of bugs, expected behavior, event payloads)
   - **Do NOT skip this step** even if description seems sufficient
4. **Select domain expert(s):**
   - Scan ticket summary, description, AC, labels, components, and comments for domain keywords (see Product Domain Experts table)
   ...
   - **Path resolution (portable across repos):** Use Glob to discover the actual path:
     ...
     ```
     This works from any repo by searching the plugin cache and local directories.
   - Extract: key business rules, expected behaviors, common test scenarios, known edge cases
   - **Do NOT use Agent tool for domain experts** -- Read tool is faster and domain files are local
   - **If `web-experimentation` is among selected domains** (or ticket labels/keywords contain `visual-editor`, `VE`, `opal-ve`, `snippet`, `change`, `variation`): also read VE docs using the Read tool.
     ```
     Glob("**/visual-editor/docs/IMPACT_INDEX.md")    → blast radius, downstream VE components
     Glob("**/visual-editor/docs/FULL_FLOW_SPEC.md")  → E2E navigation flow, test URLs, test data
     Glob("**/visual-editor/docs/snippet.md")          → change types, snippet verification methodology
     Glob("**/visual-editor/docs/API_REFERENCE.md")   → API endpoints, request/response patterns
     ```
     Extract: navigation flows, test data cụ thể (project IDs, URLs), change type verification, API call patterns, blast radius của VE components.
   - **If `opal-chat` is among selected domains** (or ticket labels/keywords contain `opal`, `opal-ve`, `opal-chat`): also read the opal-tools IMPACT_INDEX.md using the Read tool.
     ```
     Glob("**/opal-tools/docs/IMPACT_INDEX.md")
     ```
     Extract: blast radius for affected tools/services, cross-repo impacts (opal-tools ↔ visual-editor), critical paths relevant to the ticket.
5. **Dispatch optional context-gathering sub-skills IN PARALLEL using Agent tool:**
   - Only use Agent tool for tasks that genuinely need independent MCP execution:
     - Agent 1: `/common-qa:read-confluence` (if Confluence URLs found) -- needs Confluence MCP
     - Agent 2: `/common-qa:get-figma-screenshots` (if Figma URLs found) -- needs Figma MCP
     - Agent 3: `/common-qa:analyze-pr-changes` (if PRs linked) -- needs GitHub MCP
   - Set `run_in_background: true` for all agents
   - Skip agents whose MCP server is unavailable
...
9. **Generate test cases:** P0 (core AC), P1 (related), P2 (PR-discovered + domain-informed), P3 (regression) — số lượng mỗi nhóm dựa trên scope thực tế của ticket, không giới hạn cố định
10. **Present:** Full analysis with: selected domain(s), domain-informed insights, attachment analysis, test case tables, suggested next steps
...
<!-- HIGHLIGHT_START -->
> **Risk-based testing:** Effort tỷ lệ với risk + impact, không phải % cố định.
> Mỗi ticket có risk profile khác nhau — apply priority dựa trên scope, không phải quota.

| Priority | Description | When to Include |
|----------|-------------|----------------|
| P0 | Core AC (must-pass) — đúng số AC cần verify | Always |
| P1 | Directly related functionality — include all affected flows | Always |
| P2 | Broader changes from PR — add if PR scope exceeds AC | If PR touches more than AC describes |
| P3 | Regression — add only if shared utilities/components changed | Only if risk of regression exists |
<!-- HIGHLIGHT_END -->
...
| opal-tools IMPACT_INDEX.md (optional) | **skip_continue** -- File not found locally, continue without blast radius context | `{TICKET_ID}_jira_context.md` |
...
3. **Prioritize ruthlessly**: P0 là core AC cần verify — bao nhiêu AC thì bấy nhiêu P0, không giới hạn cứng
