...
5. **Review quality + coverage IN PARALLEL using Agent tool:**
   - Agent 1: Review quality -- validate each test case against quality checklist (Step 3 in detail below) + domain expert knowledge (correct navigation, UI labels, business rules)
   - Agent 2: Review coverage -- compare test cases against requirements to find gaps (Step 4 in detail below) + domain-informed scenarios
...
- **Opal + no specific product** → select `opal-chat` + all 3 products
- **Opal + specific product** → select `opal-chat` + only the mentioned product(s)
...
### Rewrite Rules (when user selects "Rewrite existing test cases")

For each test case graded C or D, apply these fixes:

1. **Add login + navigation steps:** Prepend standard login flow (see domain expert navigation patterns). Never start mid-flow.
2. **Fill ALL missing expected results:** Every step MUST have a corresponding expected result. Replace "(no expected result specified)" with concrete, verifiable outcomes.
3. **Add specific test data:** Replace vague instructions ("enter invalid data", "fill up title") with actual values ("enter '<script>alert(1)</script>'", "enter 'Test Hypothesis Alpha'").
4. **Fix copy-paste errors in preconditions:** Cross-check precondition roles against the test case title. If title says "creator in CMP" but precondition says "admin in CMP", fix the precondition.
5. **Expand thin test cases:** If a TC has only 2-4 steps, expand with proper navigation, wait steps, and verification steps.
6. **Add proper labels:** Replace single generic labels (e.g., just "Adoption") with specific functional categories (e.g., "Adoption, Validation" or "Adoption, Permission").
7. **Fix grammar in titles:** "existed hypothesis" -> "existing hypothesis", "Cancel popup" -> "Cancel and Close Create Hypothesis popup"

Preserve the original test case intent and scenario -- only improve structure, steps, and data quality.

## Error Handling

| Error | Action |
|-------|--------|
| No test cases found | Ask user to provide test cases (JIRA, text, image, Excel, or file) |
| No requirements found | Ask user to provide AC or requirements for coverage analysis. Review quality only (skip coverage) |
| Atlassian MCP not available | Accept manual input only, skip JIRA-based checks |
| Test case format unrecognizable | Ask user to clarify format or provide in a different way |
| Too many test cases (>50) | Process in batches of 20, present incremental results |
| Excel parsing fails (openpyxl missing) | Try csv fallback. If still fails, ask user to export as .csv or .md |
| Image text unreadable | Ask user to provide clearer image or paste content as text |
| Excel column mapping ambiguous | Ask user to confirm column-to-field mapping via AskUserQuestion |

## Self-Correction

1. **"Grade is too harsh/lenient for TC-X"** -> Re-evaluate with user's reasoning
2. **"AC-X is not relevant"** -> Remove from coverage analysis
3. **"Add this requirement"** -> Include in coverage analysis, re-check gaps
4. **"Only review quality, skip coverage"** -> Skip Step 4, report quality only
5. **"Only check coverage, skip quality"** -> Skip Step 3, report coverage only
6. **"Update test case in JIRA"** -> Edit the JIRA ticket with improvements
7. **"You only added new test cases, but not fixed old ones"** -> Immediately rewrite all C/D grade test cases with full quality (login steps, expected results, specific data, fixed preconditions). Do NOT wait for user to ask twice -- always offer "Rewrite existing test cases" as the first/recommended action when C/D grades exist.
